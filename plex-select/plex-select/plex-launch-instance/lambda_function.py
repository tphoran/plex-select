...
def set_user_name(intent, session):
    ...
    if id_table.query(KeyConditionExpression=Key('name').eq(owner_name))['Count'] > 0:
        ...
        speech_output = "Hi "+owner_name.title()+", welcome! "\
                        "Please enter the movie you would like to see or ask for recent releases. " \
                        "Alternatively, search for movies by saying search and the term you want to look for. "
    else:
        session_attributes = {}
